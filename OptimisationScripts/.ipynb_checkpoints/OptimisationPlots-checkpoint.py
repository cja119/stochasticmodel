...
def wind_energy(self):
    ...
    for s in self.instance.scenario:
        ...
        if self.instance.solar:
            power_solar = array([self.instance.solar_power[(st)] for st in self.instance.full_set for _ in range(st[2]) if st[0] == s])*int(self.instance.capacity_solar.value)
            time = array([(st[1],st[2]) for st in self.instance.full_set for _ in range(st[2]) if st[0] == s])                        
...
def curtailed_energy(self):
    ...
    for s in self.instance.scenario:
        time = [(st[1],st[2]) for st in self.instance.full_set for _ in range(st[2]) if st[0] == s]
        ax.plot(range(time[0][0],time[-1][0]+time[-1][1]),
                array([self.instance.energy_curtailed[(s,t)].value for t in time]),
                color = cmap[line_counter],
                linewidth = self.linewidth
                )
...
def origin_storage_tank_levels(self):
    ...
    for s in self.instance.scenario:
        ...
        for i in self.instance.vectors:
            ax.plot(range(time[0][0],time[-1][0]+time[-1][1]),
                    array([self.instance.vector_storage_origin[(s,t),i].value for t in time]),
                    label = i,
                    color = cmap[line_counter],
                    linewidth = self.linewidth
                    )
...
def LCOH_contributions(self,threshold):
    ...
    if self.instance.grid_connection.value:
        if (10**3)*sum(self.instance.net_grid[s].value/len(self.instance.scenario) for s in self.instance.scenario) * self.instance.LCOWP * self.instance.grid_energy_factor.value * self.instance.amortisation_plant / self.instance.discounted_demand >= threshold:
            categories.append("Grid Connection")
            OPEX.append((10**3)*sum(self.instance.net_grid[s].value/len(self.instance.scenario) for s in self.instance.scenario)  * self.instance.LCOWP * self.instance.grid_energy_factor.value * self.instance.amortisation_plant / self.instance.discounted_demand)
            CAPEX.append(0)
